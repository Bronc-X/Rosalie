...
export function buildInterviewMessages(request) {
  ...
  const system = [
    ...
    '面试时一次只问一个问题，每次回复控制在 120 字以内。先基于候选人的上一条回答做自然追问，再逐步覆盖业务判断、执行细节、指标、跨团队协作和危机处理。',
  ...
  const messages = [{ role: 'system', content: system }, ...request.messages];
