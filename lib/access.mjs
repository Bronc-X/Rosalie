...
export const PLAYER_COOKIE = 'rosalie_player';
export const PLAYER_TTL_MS = 365 * 24 * 60 * 60 * 1_000;
...
export async function createPlayerToken(secret, playerId, now = Date.now(), ttlMs = PLAYER_TTL_MS) {
  if (!secret) throw new Error('Access secret is required');
  if (!isPlayerId(playerId)) throw new Error('A valid player id is required');
  const expiresAt = Math.floor(now + ttlMs);
  const payload = `p1.${expiresAt}.${playerId.toLowerCase()}`;
  return `${payload}.${await hmac(payload, secret)}`;
}

export async function readPlayerId(token, secret, now = Date.now()) {
  if (typeof token !== 'string' || !secret) return null;
  const [version, expiresRaw, playerId, signature, extra] = token.split('.');
  const expiresAt = Number(expiresRaw);
  if (
    version !== 'p1'
    || extra !== undefined
  ...
  const payload = `${version}.${expiresAt}.${playerId.toLowerCase()}`;
  return constantTimeEqual(signature ?? '', await hmac(payload, secret)) ? playerId.toLowerCase() : null;
}
