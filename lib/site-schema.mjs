...
export const CREATE_HOME_NOTES_TABLE_SQL = `CREATE TABLE IF NOT EXISTS home_notes (
  id TEXT PRIMARY KEY,
  player_id TEXT NOT NULL,
...
export const CREATE_HOME_NOTES_INDEX_SQL = `CREATE INDEX IF NOT EXISTS home_notes_created_at_idx
ON home_notes(created_at DESC, id DESC)`;

export const CREATE_HOME_NOTE_REPLIES_TABLE_SQL = `CREATE TABLE IF NOT EXISTS home_note_replies (
  id TEXT PRIMARY KEY,
  note_id TEXT NOT NULL,
  player_id TEXT NOT NULL,
  text TEXT NOT NULL CHECK (length(text) BETWEEN 1 AND 100),
  created_at TEXT NOT NULL,
  FOREIGN KEY (note_id) REFERENCES home_notes(id) ON DELETE CASCADE
)`;

export const CREATE_HOME_NOTE_REPLIES_INDEX_SQL = `CREATE INDEX IF NOT EXISTS home_note_replies_note_idx
ON home_note_replies(note_id, created_at ASC, id ASC)`;
...
export const SCHEMA_STATEMENTS = [
  ...
  CREATE_HOME_NOTES_TABLE_SQL,
  CREATE_HOME_NOTES_INDEX_SQL,
  CREATE_HOME_NOTE_REPLIES_TABLE_SQL,
  CREATE_HOME_NOTE_REPLIES_INDEX_SQL,
];
...
export const SELECT_HOME_NOTES_SQL = `SELECT id, text, created_at
FROM home_notes
ORDER BY created_at DESC, id DESC LIMIT ?`;

export const INSERT_HOME_NOTE_SQL = `INSERT INTO home_notes (
  id, player_id, text, created_at
) VALUES (?, ?, ?, ?)`;

export const SELECT_HOME_NOTE_REPLIES_SQL = `SELECT r.id, r.note_id, r.text, r.created_at
FROM home_note_replies AS r
JOIN (
  SELECT id FROM home_notes
  ORDER BY created_at DESC, id DESC LIMIT ?
) AS visible_notes ON visible_notes.id = r.note_id
ORDER BY r.created_at ASC, r.id ASC`;

export const INSERT_HOME_NOTE_REPLY_SQL = `INSERT INTO home_note_replies (
  id, note_id, player_id, text, created_at
) VALUES (?, ?, ?, ?, ?)`;
